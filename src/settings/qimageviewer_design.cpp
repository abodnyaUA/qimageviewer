...
void QImageViewer::createActions()
{
    ...
    ///BUTTONS///
    imagewidget->prevButton->setToolTip(tr("Open previous image"));
    connect(imagewidget->prevButton,SIGNAL(clicked()),this,SLOT(prevImage()));
    imagewidget->prevButton->setFocusPolicy(Qt::NoFocus);

    imagewidget->nextButton->setToolTip(tr("Open next image"));
    connect(imagewidget->nextButton,SIGNAL(clicked()),this,SLOT(nextImage()));
    imagewidget->nextButton->setFocusPolicy(Qt::NoFocus);
...
void QImageViewer::createDesign()
{
    ...
    imagewidget->nextButton->setIcon(QIcon(QPixmap(iconpacks[currenticonpack] + icon["Next"])));
    imagewidget->prevButton->setIcon(QIcon(QPixmap(iconpacks[currenticonpack] + icon["Previous"])));
